...
public class MultithreadedQuicksort {
	 ...
	 *            The array to sort.
	 * @param <T>
	 *            The type of the objects being sorted, must extend Comparable.
	 */
	 ...
	 * technically recursive as it just creates new runnables and hands them off
	...
	private static class QuicksortRunnable<T extends Comparable<T>> implements
		 ...
		 *            The array to sort.
		 * @param left
		 *            The starting index of the section of the array to be
		 *            sorted.
		 * @param right
		 *            The ending index of the section of the array to be sorted.
		 * @param count
		 *            The number of currently executing threads.
		 */
		...
		/**
		 * The thread's run logic. When this thread is done doing its stuff it
		 * checks to see if all other threads are as well. If so, then we notify
		 * the count object so Sorter.quicksort stops blocking.
		 */
		@Override
		public void run() {
			quicksort(left, right);
			synchronized (count) {
				// AtomicInteger.getAndDecrement() returns the old value. If the
				// old value is 1, then we know that the actual value is 0.
		...
		/**
		 * Method which actually does the sorting. Falls back on recursion if
		 * there are a certain number of queued / running tasks.
		 * 
		 * @param pLeft
		 *            The left index of the sub array to sort.
		 * @param pRight
		 *            The right index of the sub array to sort.
		 */
		 ...
		 * inclusively, by moving all elements less than values[pivotIndex]
		 * before the pivot, and the equal or greater elements after it.
		 * 
		 * @param pLeft
		 * @param pRight
		 ...
		 * 
		 * @param left
		 *            The index of the first value to swap with the second
		 *            value.
		 * @param right
		 *            The index of the second value to swap with the first
		 *            value.
		 */
	...
	public static void main(String[] args) {
		Integer[] a = { 2, 6, 7, 8, 1, 3, 5 };
		System.out.println(Arrays.toString(a));
		MultithreadedQuicksort.quicksort(a);
	}
}
